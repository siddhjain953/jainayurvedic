...
class RetailerApp {
    ...
    calculateInsights() {
        ...
        const detailedMetrics = {
            ...
            repeatCustomers: Object.values(customers).filter(c => c.billHistory.length > 1).length,
            ...
            customerRetention: `${Object.keys(customers).length ? Math.round((Object.values(customers).filter(c => c.billHistory.length > 1).length / Object.keys(customers).length) * 100) : 0}%`,
            ...
            wishlistTotal: Object.values(customers).reduce((sum, c) => sum + c.wishlist.length, 0)
        };
    ...
    generateCustomersGrid() {
        ...
        return customers.map(customer => {
            ...
            const billCount = dataManager.getBillsByCustomer(customer.name, customer.mobile).length;
    ...
    viewCustomerFolder(name, mobile) {
        const bills = dataManager.getBillsByCustomer(name, mobile);
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div style="margin-bottom: 16px; padding: 16px; background: #f5f5f5; border: 2px solid black; border-radius: 8px;">
                            ...
                            <div style="color: black;">❤️ Wishlist: ${customer.wishlist.length} items</div>
                        </div>
