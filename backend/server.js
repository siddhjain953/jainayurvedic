...
const DEVICES_FILE = path.join(__dirname, '../data/authorized_devices.json');
...
const server = http.createServer((req, res) => {
    ...
    if (parsedUrl.pathname.startsWith('/api/')) {
        ...
        if (parsedUrl.pathname === '/retailer.html') {
            parsedUrl.pathname = '/retailer/index.html';
        } else if (parsedUrl.pathname === '/retailer-login.html') {
            parsedUrl.pathname = '/retailer/login.html';
        } else if (parsedUrl.pathname === '/customer.html') {
            parsedUrl.pathname = '/customer/index.html';
        }

        // Protect retailer pages - require device authorization
        if (parsedUrl.pathname.startsWith('/retailer/') && parsedUrl.pathname !== '/retailer/login.html') {
            if (!isDeviceAuthorized(req)) {
        ...
        let filePath = path.join(__dirname, '../frontend', requestPath);
        ...
        fs.readFile(filePath, (err, content) => {
            if (err) {
                ...
                // Development-friendly caching strategy
                // Disable cache for HTML and JS to allow immediate updates during development
                const cacheControl = (ext === '.html' || ext === '.js') ? 'no-cache' : 'public, max-age=31536000';
